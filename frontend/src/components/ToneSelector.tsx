...
const TONES = [
  { key: 'professional', label: 'Professional', icon: '👔', color: 'from-blue-600 to-blue-800', description: 'Corporate excellence' },
  { key: 'playful', label: 'Playful', icon: '🎉', color: 'from-pink-500 to-purple-600', description: 'Fun & energetic' },
  { key: 'luxury', label: 'Luxury', icon: '💎', color: 'from-yellow-400 to-orange-600', description: 'Premium quality' },
];
...
export default function ToneSelector({ value, onChange }: Props) {
  ...
  const selectedTone = TONES.find(tone => tone.key === value) || TONES[0];
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-r ${selectedTone.color} text-white shadow-lg`}>
              <span className="text-lg">{selectedTone.icon}</span>
            </div>
            <div>
              <div className="text-lg font-semibold text-gray-100">{selectedTone.label}</div>
              <div className="text-sm text-gray-400">{selectedTone.description}</div>
            </div>
          </div>
              ...
              {TONES.map((tone) => (
                <button
                  ...
                  className={`w-full flex items-center space-x-4 px-4 py-4 rounded-xl transition-all duration-200 hover:bg-gray-800 hover:scale-[1.02] ${
                    value === tone.key ? 'bg-gray-800 ring-2 ring-blue-500' : ''
                  }`}
                >
                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-r ${tone.color} text-white shadow-lg`}>
                    <span className="text-xl">{tone.icon}</span>
                  </div>
                  <div className="flex-1 text-left">
                    <div className="text-lg font-semibold text-gray-100">{tone.label}</div>
